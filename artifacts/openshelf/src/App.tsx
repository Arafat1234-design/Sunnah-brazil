...
import {
  ArrowLeft, ArrowRight, BarChart3, BookOpen, Check, ChevronDown, Mail,
  Download, FileText, Film, Headphones, Heart, Info,
  Menu, Play, Plus, Reply, Search, Send, Settings2, ShieldCheck, SlidersHorizontal,
...
function Header() {
  ...
  return <header className="sticky top-0 z-40 border-b border-[#e3e9e7] bg-[#eee6da]/95 backdrop-blur-md">
    <div className="mx-auto flex min-h-[72px] max-w-[1240px] items-center justify-between gap-5 px-5 lg:px-8">
      ...
      <button type="button" onClick={() => setOpen(!open)} className="rounded-full p-2 lg:hidden" data-testid="button-mobile-menu" aria-label={open ? 'Fechar menu' : 'Abrir menu'}>{open ? <X size={22} /> : <Menu size={22} />}</button>
    </div>
...
function LibraryToolbar({ kind, search, setSearch, category, setCategory, categories }: { kind: 'books' | 'videos'; search: string; setSearch: (v: string) => void; category: string; setCategory: (v: string) => void; categories: Category[] }) {
  return <div className="flex flex-col gap-3 border-y border-[hsl(var(--border))] py-4 sm:flex-row sm:items-center sm:justify-between"><SearchBar value={search} onChange={setSearch} placeholder={`Buscar ${kind === 'books' ? 'livros' : 'vídeos'}`} /><div className="flex items-center gap-2 overflow-x-auto"><SlidersHorizontal size={16} className="shrink-0 text-[hsl(var(--muted-foreground))]" /><button type="button" onClick={() => setCategory('')} className={`shrink-0 rounded-full px-3 py-2 text-xs ${!category ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--secondary))]'}`} data-testid="button-filter-all">Todos</button>{categories.map(c => <button type="button" key={c.name} onClick={() => setCategory(c.name)} className={`shrink-0 rounded-full px-3 py-2 text-xs ${category === c.name ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--secondary))] hover:bg-[hsl(var(--border))]'}`} data-testid={`button-filter-${c.name.toLowerCase().replace(/\s/g, '-')}`}>{c.name}</button>)}</div></div>;
}
...
function EventsPage() {
  ...
  return <Shell><main className="mx-auto max-w-[1240px] px-5 pb-20 pt-14 lg:px-8">
    ...
    <div className="mt-10 flex flex-wrap items-center gap-2 border-b border-[#dbe4e2] pb-4">
      <button type="button" onClick={() => setView('upcoming')} className={`rounded-full px-4 py-2.5 text-sm font-semibold ${view === 'upcoming' ? 'bg-[#075C45] text-white' : 'bg-[#e9f3ef] text-[#315b55]'}`} data-testid="button-events-upcoming">Próximos</button>
      <button type="button" onClick={() => setView('past')} className={`rounded-full px-4 py-2.5 text-sm font-semibold ${view === 'past' ? 'bg-[#075C45] text-white' : 'bg-[#e9f3ef] text-[#315b55]'}`} data-testid="button-events-past">Realizados</button>
    </div>
    <section className="mt-8" aria-live="polite">
...
function AdminEvents({ events, onAdd, onEdit, onDelete, onDuplicate, onTogglePublish }: { events: CatalogEvent[]; onAdd: () => void; onEdit: (event: CatalogEvent) => void; onDelete: (id: number) => void; onDuplicate: (id: number) => void; onTogglePublish: (event: CatalogEvent) => void }) {
  ...
  return <div>
    ...
    <div className="mb-5 flex gap-2 overflow-x-auto pb-1">{[['all', 'Todos'], ['upcoming', 'Próximos'], ['draft', 'Rascunhos'], ['past', 'Realizados']].map(([value, label]) => <button type="button" key={value} onClick={() => setFilter(value as typeof filter)} className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold ${filter === value ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]' : 'bg-[hsl(var(--secondary))]'}`}>{label}</button>)}</div>
    <div className="overflow-hidden rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--card))]">{filtered.length === 0 ? <StateMessage title="Nenhum evento encontrado" body={filter === 'all' ? 'Crie o primeiro evento da agenda.' : 'Não há eventos nesta categoria.'} /> : filtered.map(event => <div key={event.id} className="flex flex-col gap-4 border-b border-[hsl(var(--border))] p-4 last:border-0 sm:flex-row sm:items-center"><EventImage event={event} className="h-20 w-28 shrink-0 rounded-xl" /><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{event.title}</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{formatEventDate(event.eventDate)} · {formatEventTime(event)} · {event.city}</p><span className={`mt-2 inline-flex rounded-full px-2 py-1 text-[10px] font-semibold ${event.status === 'draft' ? 'bg-[hsl(var(--secondary))]' : event.status === 'past' ? 'bg-[#f4eadb] text-[#8a6e4b]' : 'bg-[#e5f2eb] text-[#075C45]'}`}>{statusLabel(event.status)}</span></div><div className="flex shrink-0 flex-wrap gap-1"><button type="button" onClick={() => onTogglePublish(event)} className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))]" title={event.published ? 'Despublicar' : 'Publicar'} data-testid={`button-toggle-event-${event.id}`}>{event.published ? <EyeOff size={16} /> : <Eye size={16} />}</button><button type="button" onClick={() => onDuplicate(event.id)} className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))]" title="Duplicar" data-testid={`button-duplicate-event-${event.id}`}><Copy size={16} /></button><button type="button" onClick={() => onEdit(event)} className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))]" title="Editar" data-testid={`button-edit-event-${event.id}`}><Settings2 size={16} /></button><button type="button" onClick={() => onDelete(event.id)} className="rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--secondary))] hover:text-[hsl(var(--destructive))]" title="Excluir" data-testid={`button-delete-event-${event.id}`}><Trash2 size={16} /></button></div></div>)}</div>
  </div>;
}
...
function AdminMessages() {
  ...
  const replyHref = (message: ContactMessage) => {
    const subject = /^re:/i.test(message.topic.trim()) ? message.topic.trim() : `Re: ${message.topic.trim()}`;
    const body = `Olá,\n\n\n\n--- Mensagem original ---\n${message.message}`;
