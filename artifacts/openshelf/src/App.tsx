...
function CatalogEditor({ state, setState, close, refresh }: { state: EditorState; setState: (action: SetStateAction<EditorState>) => void; close: () => void; refresh: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[hsl(193_25%_19%/.52)] p-4">
    <form onSubmit={submit} className="max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-2xl bg-[hsl(var(--card))] p-6 shadow-xl">
      <div className="flex items-center justify-between"><div><p className="mono text-[10px] uppercase tracking-[.18em] text-[hsl(var(--accent))]">{state.id ? 'Edit item' : 'New item'}</p><h2 className="serif mt-1 text-3xl">{state.kind === 'book' ? 'Add a book' : 'Add a video'}</h2></div><button type="button" onClick={close} data-testid="button-close-editor"><X size={20} /></button></div>
      <div className="mt-6 grid gap-4">
        <label className="text-sm font-semibold">Title<input required value={state.title} onChange={e => patch('title', e.target.value)} className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="input-editor-title" /></label>
        {state.kind === 'book' && <label className="text-sm font-semibold">Author<input required value={state.author} onChange={e => patch('author', e.target.value)} className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="input-editor-author" /></label>}
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="text-sm font-semibold">Category<input required value={state.category} onChange={e => patch('category', e.target.value)} className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="input-editor-category" /></label>
          {state.kind === 'book' ? <label className="text-sm font-semibold">Format<select value={state.fileType} onChange={e => patch('fileType', e.target.value)} className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="select-editor-format"><option>PDF</option><option>EPUB</option><option>MOBI</option><option>TXT</option></select></label> : <label className="text-sm font-semibold">Duration<input required value={state.duration} onChange={e => patch('duration', e.target.value)} placeholder="38 min" className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="input-editor-duration" /></label>}
        </div>
        <label className="text-sm font-semibold">Description<textarea required rows={4} value={state.description} onChange={e => patch('description', e.target.value)} className="mt-2 w-full rounded-lg border bg-transparent p-3 text-sm" data-testid="textarea-editor-description" /></label>
        {state.kind === 'book' ? <div className="grid gap-4 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--secondary)/.35)] p-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-semibold"><span className="flex items-center gap-2"><UploadCloud size={15} />Video file</span><input type="file" accept="video/*,.mp4,.webm,.mov" disabled={busy} className="mt-2 block w-full text-xs" onChange={e => { const file = e.target.files?.[0]; e.currentTarget.value = ''; if (file) void uploadFile(file, 'videoUrl'); }} data-testid="input-editor-video-upload" /><span className="mt-2 block text-xs font-normal text-[hsl(var(--muted-foreground))]">{uploading === 'video' ? 'Uploading video and creating cover…' : state.videoUrl ? 'Video ready to save' : 'Upload a video to generate its title and duration'}</span></label>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-semibold"><span className="flex items-center gap-2"><UploadCloud size={15} />Upload cover</span><input type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={busy} className="mt-2 block w-full text-xs" onChange={e => { const file = e.target.files?.[0]; e.currentTarget.value = ''; if (file) void uploadFile(file, 'thumbnailUrl'); }} data-testid="input-editor-video-cover-upload" /><span className="mt-2 block text-xs font-normal text-[hsl(var(--muted-foreground))]">{uploading === 'cover' ? 'Uploading cover…' : state.thumbnailUrl ? 'Cover ready' : 'Optional if the first video frame is used'}</span></label>
            <label className="text-sm font-semibold">Choose from gallery<select value={state.thumbnailUrl ?? ''} onChange={e => patch('thumbnailUrl', e.target.value)} className="mt-2 h-11 w-full rounded-lg border bg-transparent px-3 text-sm" data-testid="select-editor-video-gallery-cover"><option value="">Use generated cover</option>{(galleryImages.data ?? []).map(image => <option key={image.id} value={image.imageUrl}>{image.title}</option>)}</select><span className="mt-2 block text-xs font-normal text-[hsl(var(--muted-foreground))]">Select an existing public image as the cover.</span></label>
          </div>
          {state.thumbnailUrl && <img src={state.thumbnailUrl} alt="Video cover preview" className="max-h-48 w-full rounded-xl object-contain bg-[hsl(var(--secondary))]" />}
        </div>}
        {uploadError && <p className="text-sm text-[hsl(var(--destructive))]" role="alert">{uploadError}</p>}
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={state.featured ?? false} onChange={e => patch('featured', e.target.checked)} data-testid="checkbox-editor-featured" /> Feature this item</label>
        {state.kind === 'video' && <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={state.downloadEnabled ?? false} onChange={e => patch('downloadEnabled', e.target.checked)} data-testid="checkbox-editor-download" /> Allow downloads</label>}
      </div>
      <div className="mt-7 flex justify-end gap-2"><Button type="button" variant="ghost" onClick={close}>Cancel</Button><Button type="submit" disabled={busy}>{busy ? 'Saving…' : 'Save to catalog'}</Button></div>
    </form>
  </div>;
}
