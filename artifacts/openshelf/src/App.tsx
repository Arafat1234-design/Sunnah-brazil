...
function Cover({ book, large = false }: { book: Book; large?: boolean }) {
  return <div className={`book-cover ${large ? 'aspect-[3/4] max-w-[280px] rounded-2xl' : 'aspect-[3/4] rounded-[14px]'} shadow-sm`}>
    {book.coverUrl ? <img src={book.coverUrl} alt={book.title} className="relative z-[1] h-full w-full object-cover" /> : <div className="relative z-[1] flex h-full flex-col justify-between p-5 text-[hsl(var(--primary-foreground))]"><span className="mono text-[10px] uppercase tracking-[.15em] opacity-75">Edição Sunnah Brasil</span><div><h3 className={`serif leading-[1.02] ${large ? 'text-3xl' : 'text-xl'}`}>{book.title}</h3><p className="mt-2 text-xs opacity-75">{book.author}</p></div></div>}
  </div>;
}
...
function Videos() {
  ...
  return <Shell><main className="mx-auto max-w-[1240px] px-5 pb-16 pt-14 lg:px-8"><div className="mb-10"><p className="mono text-[10px] uppercase tracking-[.2em] text-[hsl(var(--accent))]">A sala de exibição</p><h1 className="serif mt-3 text-5xl tracking-[-.04em] md:text-6xl">Assista com intenção.</h1><p className="mt-4 max-w-lg text-[hsl(var(--muted-foreground))]">Filmes independentes, conversas atentas e imagens em movimento com um ponto de vista.</p></div><LibraryToolbar kind="videos" search={search} setSearch={setSearch} category={category} setCategory={setCategory} categories={cats.data ?? []} /><div className="mt-8">{q.isLoading ? <LoadingGrid kind="video" /> : q.isError ? <StateMessage error title="A sala de exibição está silenciosa" body="Tente novamente daqui a pouco." retry={q.refetch} /> : !q.data?.length ? <StateMessage title="Nenhum filme corresponde à busca" body="Tente outra frase ou explore todas as categorias." /> : <div className="grid gap-x-5 gap-y-10 sm:grid-cols-2 lg:grid-cols-3">{q.data.map(video => <VideoCard key={video.id} video={video} />)}</div>}</div></main></Shell>;
}
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-[hsl(193_25%_19%/.52)] p-4">
    <form onSubmit={submit} className="max-h-[90dvh] w-full max-w-xl overflow-y-auto rounded-2xl bg-[hsl(var(--card))] p-6 shadow-xl">
      ...
      <div className="mt-6 grid gap-4">
        ...
        {uploadError && <p className="text-sm text-[hsl(var(--destructive))]" role="alert">{uploadError}</p>}
  ...
  return <div className="min-w-0 max-w-full space-y-6">
    ...
    {analytics.isLoading ? <AnalyticsSkeleton /> : analytics.isError ? <StateMessage error title="Não foi possível carregar as análises" body="Tente novamente em alguns instantes." retry={analytics.refetch} /> : data ? <>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <AnalyticsCard eyebrow="Registro" title="Atividade recente" className="bg-white">{data.recentActivity.length ? <div className="mt-5 divide-y divide-[hsl(var(--border))]">{data.recentActivity.slice(0, 7).map((item, index) => <div key={`${item.createdAt}-${index}`} className="flex items-start justify-between gap-4 py-3 first:pt-0 last:pb-0"><div className="flex items-start gap-3 min-w-0"><span className="mt-0.5 text-[hsl(var(--muted-foreground))]">{item.type === 'book' ? <BookOpen size={14}/> : item.type === 'video' ? <PlayCircle size={14}/> : item.type === 'image' ? <Images size={14}/> : <CalendarDays size={14}/>}</span><div className="min-w-0"><p className="truncate text-sm">{item.title}</p><p className="mt-1 text-xs text-[hsl(var(--muted-foreground))]">{item.type === 'book' ? 'Livro' : item.type === 'video' ? 'Vídeo' : item.type === 'image' ? 'Imagem' : 'Evento'}</p></div></div><div className="flex shrink-0 flex-col items-end gap-1"><span className="text-[11px] text-[hsl(var(--muted-foreground))]">{formatDate(item.createdAt, true)}</span></div></div>)}</div> : <div className="mt-5"><EmptyAnalytics label="Ainda não há atividade recente." /></div>}</AnalyticsCard>
      </div>
      <div className="grid gap-5 lg:grid-cols-2">
